...
export default async function CategoryPage({ params }: { params: any }) {
  ...
  // Lấy các mục Con để làm Menu (lọc bỏ thằng Mẹ ra khỏi danh sách nút bấm)
  const menuTabs = (CATEGORY_TREE[parentKey] || []).filter(sub => sub !== parentKey);
  
  // 2. Phân loại lấy bài: Nếu đang ở Mẹ -> lấy hết họ hàng. Nếu đang ở Con -> chỉ lấy Con.
  let searchIds: string[] = [];
  if (currentCat === parentKey) {
    searchIds = CATEGORY_TREE[parentKey]; 
  } else {
    searchIds = [currentCat];
  }
  
  // Khớp database có dấu/không dấu
  if (searchIds.includes('tho')) searchIds.push('thơ');
  if (searchIds.includes('tan-van')) searchIds.push('tản văn');
  if (searchIds.includes('den')) searchIds.push('đền');
  if (searchIds.includes('gieng')) searchIds.push('giếng');
      ...
      <div style={{ maxWidth:1160, margin:"0 auto", padding:"0 16px" }}>
        ...
        <div style={{ background:"linear-gradient(135deg,#9B1B14,#B91C1C)", color:"#fff",
          ...
          <div>
            ...
            <p style={{ opacity:.8, fontSize:13, marginTop:4 }}>{artsList.length} bài viết nội dung</p>
          </div>
        </div>
        ...
        {artsList.length > 0 ? (
          ...
          <div style={{ textAlign:"center", padding:"100px 0", border:"2px dashed #E8DDD0", borderRadius:12 }}>
            <p style={{ color:"#888" }}>Chưa có nội dung cho chuyên mục này.</p>
          </div>
        )}
      </div>
      <Footer />
