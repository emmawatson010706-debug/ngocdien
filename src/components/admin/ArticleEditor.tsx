...
export default function ArticleEditor({ article, categories }: Props) {
  ...
  const uploadImage = async (file: File) => {
    ...
    if (!res.ok) { alert('Lỗi upload: ' + (json.error ?? 'Không upload được file')); return; }
  ...
  const save = async (publish?: boolean) => {
    ...
    setSaving(false);
    if (!res.ok) { setError(json.error ?? 'Không lưu được bài viết'); return; }
    ...
    <div className="max-w-[920px]">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[1fr_280px] gap-5">
        ...
        <div className="space-y-4">
          ...
          {/* Source */}
          <div className="bg-white border border-gray-200 rounded-xl p-4">
            <label className="admin-label">Nguồn tư liệu</label>
            <textarea value={form.source_note} onChange={e => set('source_note', e.target.value)}
              rows={3} placeholder="Ví dụ: Ban biên tập tổng hợp từ gia đình, hương ước, lời kể nhân chứng..."
              className="admin-input resize-none" />
          </div>

          {/* Document type */}
          <div className="bg-white border border-gray-200 rounded-xl p-4">
            <label className="admin-label">Loại tư liệu</label>
            <select value={form.document_type} onChange={e => set('document_type', e.target.value)}
              className="admin-input">
              <option value="">— Không chọn —</option>
              <option value="huong-uoc">Hương ước</option>
              <option value="anh-cu">Ảnh cũ</option>
              <option value="van-ban">Văn bản</option>
              <option value="gia-pha">Gia phả</option>
              <option value="ky-uc">Ký ức</option>
            </select>
          </div>
          ...
          <div className="bg-white border border-gray-200 rounded-xl p-4 space-y-3">
            ...
            {[
              { key:'is_featured', label:'⭐ Bài nổi bật (hiện carousel trang chủ)' },
